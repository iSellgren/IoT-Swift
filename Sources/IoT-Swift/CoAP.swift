...
class CoAP {
    ...
    func start (){
        ...
        let version = (VersionInput) << 2
        let type = (TypeInput) << 0
        let tkl:UInt8 = 0
        ...
        let CodeInput:UInt8 =  UInt8(Int(readLine()!)!)
        ...
        let Opt:UInt8 = UInt8(((OptInputDelta << 4 ) + (OptInputLenght)))
        
        
        
        var hexArray:[UInt8] = [(firstMessageByte), (CodeInput), (MessageIDFirst), (MessageIDSecond), (Opt)]
        ...
        let readBuffer: UnsafeMutablePointer<UInt8> = UnsafeMutablePointer<UInt8>.allocate(capacity: bufferSize)
